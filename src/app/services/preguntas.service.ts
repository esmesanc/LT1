...
export class PreguntasService {
  ...
  constructor(private db: AngularFirestore) {
    ...
    this.preguntas = this.preguntasCollection.snapshotChanges().pipe(
      map((actions) => {
  ...
  getpregunta(topic: string, dificultad: string) {
    this.preguntasCollection = this.db.collection<preguntitas>(
      'preguntitas',
      (ref) =>
        ref.where('dificultad', '==', dificultad).where('topic', '==', topic)
    );
    this.preguntas = this.preguntasCollection.snapshotChanges().pipe(
      map((actions) => {
        return actions.map((a) => {
